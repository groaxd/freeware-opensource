...
public class FastPlaceTransformer extends Transformer {
    public void transform(final ClassReader classReader, final ClassNode classNode) {
        if (classNode.name.equals("net/minecraft/client/br")) {
            for (final MethodNode methodNode : classNode.methods) {
                if (methodNode.name.equals("a") && methodNode.desc.endsWith("Z") && BG(methodNode.desc) == 6) {
                    ...
                    methodNode.instructions.insertBefore(abstractInsnNode, new JumpInsnNode(167, labelNode));
    ...
    private static int BG(final String s) {
